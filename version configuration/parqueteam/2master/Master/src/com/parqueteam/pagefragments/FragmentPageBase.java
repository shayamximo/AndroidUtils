...
import com.parqueteam.actionbar.ActionBarContorller;
import com.parqueteam.actionbar.ActionBarControllerFactory;
...
//			actionBarContorller.textViewFarRight.setTypeface(font);
	...
	protected boolean shouldCustomizeEmptyList(
		...
		if (customizedEmptyListFragment.isEmptyList()) {
			...
			if ((staticBackground != null&&staticBackground.length()>0) || (staticImage != null&&staticImage.length()>0)) {
				return true;
			}
		}
	...
	protected void setSaleViewColor(View view)
	{
		String saleColor = InitInfoStore.getInstance().getSaleColor();
		setViewToColor(view,saleColor);
	}
	
	protected void setSaleTextViewColor(TextView textView)
	{
		String saleColor = InitInfoStore.getInstance().getSaleColor();
		textView.setTextColor(Color.parseColor(saleColor));
	}
