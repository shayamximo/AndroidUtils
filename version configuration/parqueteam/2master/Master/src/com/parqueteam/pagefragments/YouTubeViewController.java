...
public class YouTubeViewController extends FragmentPageBase {
	...
	public String getThumbnail(JSONObject jsonObject, boolean isPlaylist)
			throws JSONException {
		if (isPlaylist) {
			return jsonObject.getJSONObject("video").getJSONObject("thumbnail")
					.getString("hqDefault");

		} else {
			return jsonObject.getJSONObject("thumbnail").getString("hqDefault");

		}

	}
	
	public String getId(JSONObject jsonObject, boolean isPlaylist)
			throws JSONException {
		if (isPlaylist) {
			return jsonObject.getJSONObject("video").getString("id");
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					String urlForGettingVideoInformation = null;
					urlForGettingVideoInformation = "https://gdata.youtube.com/feeds/api/playlists/"
					...
					if (playListID.length() == urlOfPlaylist.length()) {
						...
						urlForGettingVideoInformation = "https://gdata.youtube.com/feeds/api/videos?alt=jsonc&v=2&author="
								+ playListID;
						isPlaylist = false;
					}
					...
					for (int i = 0; i < jsonArray.length(); i++) {
						...
						try {
							...
							String id = getId(jsonObject, isPlaylist);
							String thumbnail = getThumbnail(jsonObject, isPlaylist);
