...
//				textOfBuy.setTypeface(font);
	...
	private void initializeSimpleFields(String title, Product product) {
		...
		if (salePrice == null) {
			...
			setSaleViewColor(textFinalPrice);
		}
	}
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		configureRatingBarColor();
		configureAvgOnRatingView();
		super.onActivityCreated(savedInstanceState);

	}

	private void configureAvgOnRatingView() {
		TextView textOfAverage = (TextView) viewOfFragment
				.findViewById(R.id.reviews_average);
		
		float avgRatingFloat = master.getAverageRating();
		int reviewsCountFloat = master.getReviewsCount();
		
		if (avgRatingFloat>0)
		{
			String avgRatingString = Float.toString(avgRatingFloat);
			String reviewsCountString = Integer.toString(reviewsCountFloat);
			textOfAverage.setText(avgRatingString + " (based on " + reviewsCountString +  " reviews)");
		}
		

	}

	private void configureRatingBarColor() {
		RatingBar rb = (RatingBar) viewOfFragment
				.findViewById(R.id.com_iinfo_RatingBar);

		LayerDrawable stars = (LayerDrawable) rb.getProgressDrawable();
		stars.getDrawable(2).setColorFilter(Color.parseColor("#8acf39"),
	...
	private void buildOtherProducts(final String title,
		...
		for (Product product : otherProdcutsList) {
			...
			if (!showOnlyChosenProduct&&(otherProdcutsList.size()>1)) {
					...
					new SimpleImageLoadingListener() {
						...
						public void onLoadingComplete(String imageUri,
							...
							if (isOnSale) {
								...
								if (InitInfoStore.getInstance()
										.showDiscountBadge()) {

									GradientDrawable backgroundGradient = (GradientDrawable) saleCircle
											.getBackground();
									backgroundGradient.setColor(Color
											.parseColor(InitInfoStore
													.getInstance()
													.getBrandColor()));
